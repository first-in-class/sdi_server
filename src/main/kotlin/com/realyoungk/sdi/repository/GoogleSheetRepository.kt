...
class GoogleSheetRepository {
    companion object {
        private const val APPLICATION_NAME = "sdi-server"
        private const val CREDENTIALS_FILE_PATH = "firstinvest-google-spread-sheet-key.json"
    }

    @Throws(IOException::class, GeneralSecurityException::class)
    fun readSheet(spreadsheetId: String, range: String): List<List<Any>>? {
        val sheets = getSheets()
        val response: ValueRange = sheets.spreadsheets().values().get(spreadsheetId, range).execute()
    ...
    private fun getSheets(): Sheets {
        val credentials = GoogleCredentials
            .fromStream(ClassPathResource(CREDENTIALS_FILE_PATH).inputStream)
